...
$resresc1 = mysqli_query("SELECT MAX(treid) FROM treatment");
while($rown = mysqli_fetch_array($resresc1))
  	{
$countrec = $rown[0];
if($rown[0] ==0)
{
...
$resresc = mysqli_query("SELECT MAX(treid) FROM treatment");
while($row = mysqli_fetch_array($resresc))
  	{
	$treid =   $row[0];
 	}
...
$dt=date("d-m-Y");
mysqli_query("UPDATE appointment SET status='$dt' WHERE patid='$_POST[patid]' ");
...
<input name="testid" type="text" class="validate[required] text-input" id="textfield" value="<?php echo $countrec; ?>" readonly="readonly"/>
  ...
  <p><strong>Lab Test Type
      (if required)
      <?php
         $res123 = mysqli_query("SELECT * FROM testtype");
		 ?>
