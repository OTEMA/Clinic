...
<?php
include("menu.php"); 
$resapp1 = mysqli_query("SELECT * FROM appointment where patid  ='$_SESSION[patid]'");
$i = 0;
 while($row2 = mysqli_fetch_array($resapp1))
  {
...
for($j = 0; $j<$i; $j++)
{
		  $resapp1 = mysqli_query("SELECT * FROM treatment where appointid='$trec[$j]'");
          while($row2 = mysqli_fetch_array($resapp1))
		{
	  ?>
          <tr>
            ...
            <?php
            $resapp1 = mysqli_query("SELECT * FROM doctor where docid='$row2[docid]'");
          		while($row3 = mysqli_fetch_array($resapp1))
				{
			?>
