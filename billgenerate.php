...
include("functions/emp.php");
 $dataa1 = date("Y-m-d"); 
$resultpatientrec = mysqli_query("SELECT * FROM patient WHERE patid ='$_GET[patid]'");
...
$gender = $row[gender];
 ...
 $respacdoc = mysqli_query("SELECT MAX(appointid) FROM appointment where patid='$_GET[patid]'");
...
$appid = $row26a[0];
 	...
 	$resp = mysqli_query("SELECT * FROM treatment where appointid='$appid'");
...
$gen = 0;
	$resp = mysqli_query("SELECT MAX(billid) FROM billing");
 while($rowa0 = mysqli_fetch_array($resp))
	{
 $billno = $rowa0[0];

	}

 $respacdoc = mysqli_query("SELECT * FROM labtest where patid='$_GET[patid]'");
 while($row26a = mysqli_fetch_array($respacdoc))
{
...
<div id="container">
 ...
 <table width="1000" border="1" align="center">
   ...
   <?php
   	  $respacdcx = mysqli_query("SELECT * FROM billing where patid='$_GET[patid]'");
 while($rowcx = mysqli_fetch_array($respacdcx))
{
...
$name[$i] = $wordChunks[$i] ;
   ...
   $respabc1 = mysqli_query("SELECT * FROM appointment where patid='$_GET[patid]' AND adate='$dataa1'");
 $ip =0;
 $treatfee =0;
 $treidid="";
 while($rowabc = mysqli_fetch_array($respabc1))
	{
   ...
   {
 	$respabc2 = mysqli_query("SELECT * FROM treatment where appointid='$appids[$ips]'");
 	while($rowxyz = mysqli_fetch_array($respabc2))
	{
	$treid = $rowxyz["treid"];
...
?>
   <tr>
     <td><?php  
 $respacdoc = mysqli_query("SELECT * FROM doctor where docid='$docid'");
 while($row26ab = mysqli_fetch_array($respacdoc))
{
$docname = $row26ab["doctorname"];
   ...
   <tr>
    ...
    $totalfee=0;
		   ...
		   $kabrec= mysqli_query("SELECT * FROM labtest where patid='$_GET[patid]' AND date = '$dataa1'");
          while($rowlab = mysqli_fetch_array($kabrec))
  {
   ...
   <tr>
     <td>&nbsp;	<?php	
$labfee = 0;
	  $restype = mysqli_query("SELECT * FROM testtype where ttypeid 	='$rowlab[ttypeid]'");
	 while($roweb = mysqli_fetch_array($restype))
		{
 ...
 $labfee = $labfee + $rowlab["labfee"]
  ...
  ?>
   ...
   <tr>
     <td colspan="2">&nbsp;
     <?php
     $res90 = mysqli_query("SELECT * FROM billing where treid ='$treidids'");
$trd = mysqli_num_rows($res90);
if($trd>0)
{
