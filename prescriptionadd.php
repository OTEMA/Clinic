...
if(isset($_GET["patid"]))
{
  ...
  $resultac = mysqli_query("SELECT * FROM treatment where appointid='$apid'");
while($rowd = mysqli_fetch_array($resultac))
  {
$treid= $rowd["treid"];
  }
}
...
if ( $_POST['formsubmissionId'] == $_SESSION['nextValidSubmission'])
{
	...
	{
	//mysqli_query("UPDATE prescription SET medicine = ( medicine,  ' $_POST[med]') WHERE treid = '$_POST[treid]'");
  ...
  if(isset($_GET["presdelid"]))
	{
		$patids = $_GET["patid"]; 
	$treid = $_GET["treid"];
	mysqli_query("DELETE FROM prescription WHERE presid='$_GET[presdelid]'");
	}
?>
<!-- ####################################################################################################### -->
...
<table width="561" border="1" bgcolor="124240">
  <tr>
  ...
  <tr>
    ...
    <td>Name
      <?php
         $res123 = mysqli_query("SELECT * FROM medicine");
		 ?>
      <select name="med" id="med" class="validate[required] text-input" >
        <option value="">Select</option>
     <?php  
     while($rowf = mysqli_fetch_array($res123))
  	{
echo "<option value='$rowf[medicine]'>$rowf[medicine]</option>";
  ...
  <tr>
    <td colspan="2"><table width="775" border="1">
      ...
      <?php
	   $resultcs = mysqli_query("SELECT * FROM prescription where patid='$patids' AND treid='$treid'");
	 // echo $_SESSION["abd"];
	  while($rowdq = mysqli_fetch_array($resultcs))
  {
