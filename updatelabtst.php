...
$resultpatientrec = mysqli_query("SELECT * FROM labtest where testid='$_GET[testid]'");
...
$empid = $row["empid"];
 ...
 $comment = $row["comment"];
  ...
  $resultttype = mysqli_query("SELECT * FROM testtype where ttypeid='$ttypeid'");
while($row = mysqli_fetch_array($resultttype)) 
  {
...
$dt= date("Y-m-d");
$resrec= mysqli_query("UPDATE labtest SET labfee ='$_POST[labfee]', date ='$dt',time='$_POST[time]',comment='$_POST[comment]',empid='$_SESSION[empid]' where testid='$_POST[testid]'");
}
